...
int main(int argc, char * argv[]) {
    ...
    // Set-up SSL with error checking
    if (SSL_library_init() < 0 ) {
        fprintf(stderr, "Error: Failure with SSL_library_init()\n");
        exit(2);
    }
    ...
    if (newContext == NULL) {
        fprintf(stderr, "Error: Issue with SSL_CTX_new()\n");
        exit(2);
    }
    
    ssl = SSL_new(newContext);
    if (ssl == NULL) {
        fprintf(stderr, "Error: Issue with SSL_new()\n");
        exit(2);
    }
    if (SSL_set_fd(ssl, sockFD) != 1) {
        fprintf(stderr, "Error: Issue with SSL_set_fd()\n");
        exit(2);
    }
    ...
    while (1) {
        ...
        if (pollfd[0].revents & POLLIN) {
            ...
            char buf[256];
            memset(buf, 0, 256);
            int bytesRead = SSL_read(ssl, &buf, 256);
            if (bytesRead < 0) {
