...
/*
 Things to crosscheck with Megan:
 [ ] - SUPERBLOCK and GROUP definitions/names
 [ ] - superBlockSum name
 [ ] - Header files included
 [ ] - IMPORTANT: DELETE ALL "DELETE ME" comments
 */
...
void handleIndirectPrint(int inode, int blockOffset, int level, int blockNumber) {
    ...
    while (index < address) {
    /*  [DELETE ME]
        IMPORTANT!! DELETE ME BEFORE YOU TURN ME IN !! @@@@@@@@@@@@@@@@@@@@@@@@@@@@
        The if/else statement below this fat comment used to be a the
        switch statement saved here. If code is broken, maybe change back
        to a switch statement
     
     switch(readBlock[index]) {
...
void handleInodes() {
    ...
    for(unsigned int k = 0; k < sb.s_inodes_count; k++) {
        ...
        if (inode.i_links_count != 0 && inode.i_mode !=0) {
            ...
            strftime(cTime, 50, "%m/%d/%y %H:%M:%S", gmt);
            ...
            strftime(mTime, 50, "%m/%d/%y %H:%M:%S", gmt);
            ...
            strftime(aTime, 50, "%m/%d/%y %H:%M:%S", gmt);
            ...
            // Handle the next 15 addresses as needed
