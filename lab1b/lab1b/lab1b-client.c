    ...
    while (1) {
        ...
        if (ret > 0) {
            ...
            if (fds[0].revents & POLLIN) {
                ...
                for (k = 0; k < bytesRead; k++) {
                    ...
                    if (buffer[k] == CR || buffer[k] == NL) {
                        ...
                        if (compressFlag) {
                            ...
                            if (logFlag) {
                                ...
                                ssize_t check2 = write(logFile, &buffer[k], 1);
                    ...
                    // If not \r or \n
                        ...
                        // use compressBuffer() function. Else, write it normally.
                        if (compressFlag) {
                            compressBuffer(&buffer[k], sockfd);
                        }
                        else {
                            ssize_t check1 = write(sockfd, &buffer[k], 1);
                            if (check1 < 0) {
                                fprintf(stderr, "Error writing to socket: %s\n", strerror(errno));
                                exit(1);
                            }
            ...
            // Check if the client has any input from server
                ...
                // Read into buffer. Check for errors
                memset(buffer, 0, 256);
                int bytesRead = read(sockfd, buffer, 256);
                if (bytesRead < 0) {
                ...
                if (logFlag) {
                    ...
                    // to in avoid errors
                    char prefix[256];
                    snprintf(prefix, 256, "RECEIVED %d bytes: ", bytesRead);
                ...
                if (compressFlag) {
                    decompressBuffer(buffer, bytesRead);
                }
...
void compressBuffer(char* bufferToCompress, int socket) {
    ...
    // avail_in = # bytes available in next_in, next_in = next input byte
    // avail_out = Remaining free space at next_out, next_out = next output byte
    // Need (unsigned char*) or XCode threw up an error
    strm.avail_in = 1;
    strm.next_in = (unsigned char*)bufferToCompress;
    strm.avail_out = 256;
    strm.next_out = (unsigned char*)compressedBuffer;
